...
(function() {
    ...
    function generateMAC(method, id, key, hostWithPort, url) {
        ...
        console.debug(id + ' | ' + hostNoPort + ' | ' + port + ' | ' + url + ' | ' + method + ' | ' + key);
        ...
        sync : function(method, model, options) {
            ...
            function _prepareAuth(method, params) {
                ...
                && macKey) {
                    ...
                    StackMob.METHOD_MAP[method]
                    || 'GET', accessToken,
                    macKey, StackMob['API_SERVER'], path);
                    if (authHeaders)
    ...
    var createStackMobModel = function() {
        ...
        .extend({
            ...
            save : function(key, value, options) {
              ...
              //This is to clear out the unusual scenario where if you try to update a model and it has a URL in the binary field, it will try to update it on the server onsave.  We keep a flag of whether or not they've called setBinaryFile so that if it exists, we allow the update in the request body.  If it does not have binary data, then we will remove it.  Clean up the boolean here after we save.
              options['stackmob_onupdate'] = function(result) {
