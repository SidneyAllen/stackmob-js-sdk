...
(function() {
  ...
  var thisWindow = typeof window !== "undefined" ? window : {
    location: {
      hostname: 'api.stackmob.com',
      port: '443',
      protocol: 'https:'
    }
  };
  ...
  thisWindow.StackMob = root.StackMob = {
    ...
    getProtocol: function() {
      return thisWindow.location.protocol;
    },
    getHostname: function() {
      return thisWindow.location.hostname;
    },
    getPort: function() {
      return thisWindow.location.port;
    },
    ...
    _containsCallbacks: function(options, callbacks) {
      return (typeof options === "object") && _.some(callbacks, function(callback) {
  ...
  var createStackMobUserModel = function() {
    ...
    StackMob.GeoPoint = function(lat, lon) {
      if (_.isNumber(lat)) {
        // Validate
        if (lat < -90 || lat > 90) {
          StackMob.throwError("Latitude value must be between -90 and 90 inclusive.");
        }
        if (lon < -180 || lon > 180) {
          StackMob.throwError("Longitude value must be between -180 and 180 inclusive.");
        }
